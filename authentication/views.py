...
class Register(View):
    ...
    def post(self, request):
        ...
        if User.objects.filter(username=username).exists():
            return render(request, 'authentication/Register.html', {'error_message': 'Username already exists. Please choose a different one.'})

        try:
            user = User.objects.create_user(
                username=username, password=password)
        except Exception as e:
            return render(request, 'authentication/Register.html', {'error_message': f'Error creating user: {str(e)}'})
